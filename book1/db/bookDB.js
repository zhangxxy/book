...
module.exports = {
    ...
    //通过id删除书籍
    DeleteById(Book_num){
      var sql="delete from t_book where Book_num= " +Book_num;
      return pool.execute(sql);
    },
    //删除多个书籍,需要做测试
    batchDelete(ids){
      var sql = "delete from t_book where Book_num in ("+ids.join()+")";
      return pool.execute(sql);
    },
    //添加书籍信息
    save(book){
      var sql = "insert into t_book values(null,'"+book.Book_name+"','"
      +book.Writer+"','"
      +book.Sort_id+"','"
      +book.Price+"','"
      +book.Pub_company+"','"
    ...
    //修改书籍信息
    update(book){
      var sql = "update t_book set Book_name = '"
